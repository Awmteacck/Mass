...
import os
token = '7175703585:AAHMGHu_ggfNh1D5EZakddJ0OeKK80AI5uI' #bottoken
bot=telebot.TeleBot(token,parse_mode="HTML")
subscriber = '7237320756'
allowed_users = ['7237320756']  #Your ID
@bot.message_handler(commands=["start"])
def start(message):
    if str(message.chat.id) not in allowed_users:
        bot.reply_to(message, "🚫 𝐘𝐨𝐮 𝐜𝐚𝐧𝐧𝐨𝐭 𝐮𝐬𝐞 𝐭𝐡𝐞 𝐛𝐨𝐭 𝐭𝐨 𝐜𝐨𝐧𝐭𝐚𝐜𝐭 𝐝𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫𝐬 𝐭𝐨 𝐩𝐮𝐫𝐜𝐡𝐚𝐬𝐞 𝐚 𝐛𝐨𝐭 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐩𝐭𝐢𝐨𝐧 @Awmtee")
        return
    bot.reply_to(message, "𝐒𝐞𝐧𝐝 𝐭𝐡𝐞 𝐭𝐱𝐭 𝐟𝐢𝐥𝐞 𝐧𝐨𝐰")
@bot.message_handler(commands=["add"])
def add_user(message):
    if str(message.chat.id) == '7237320756':  # Only bot owner can add new users
        try:
...
def generate_code(message):
    if str(message.chat.id) == '7237320756':  # Only the bot owner can generate codes
